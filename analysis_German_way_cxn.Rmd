...
left_join(collex_RBK,
          select(collex(as.data.frame(d_tbl), corpsize = sum(dwds$Freq), am = "odds"), COLLEX, COLL.STR.ODDS)) %>% DT::datatable()

#%>% write.xlsx("simple_collexeme_analysis_reflexive_Bewegungskonstruktion.xlsx")
...
# collexeme analysis with G2
collex_RPV <- collex(as.data.frame(d_tbl), corpsize = sum(dwds$Freq)) 

# add further association measures
left_join(collex_RPV,
          select(collex(as.data.frame(d_tbl), corpsize = sum(dwds$Freq), am = "odds"), COLLEX, COLL.STR.ODDS)) %>% DT::datatable()
